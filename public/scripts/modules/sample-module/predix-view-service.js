/*global define */
define(['angular', './sample-module'], function (angular, module) {
    'use strict';
    /**
    * PredixViewService is a sample angular service that integrates with Predix View Service API
    */
    module.factory('PredixViewService', ['$http', '$q', function ($http, $q) {
        return {
            getDecksByTags: function (tags) {
                var deferred = $q.defer();
                $http.get('/api/view-service/decks/tags?values=' + tags)
                    .then(function (res) {
                        deferred.resolve(res.data);
                    },
                    function () {
                        deferred.reject('Error fetching decks with tags ' + tags);
                    });
                return deferred.promise;
            },
            getUrlForFetchingCardsForDeckId: function(deckId) {
                return '/api/view-service/decks/' + deckId + '?filter[include][cards]';
            },
            
            getCoreCompetencyReport: function(successHandler,errorHandler) {
         	   debugger;
                var deferred = $q.defer();
                var baseUrl = 'https://ojtdevservice.run.asv-pr.ice.predix.io/service/get/adminCoreCompetencyReport'; 
               //https://ojtdevservice.run.asv-pr.ice.predix.io/service/get/adminCoreCompetencyReport
                $http.get(baseUrl)
                       .then(function (res) {
                      	 successHandler(res.data);
                      	 deferred.resolve(res.data);
                       },
                       function (res) {
                      	 errorHandler(res.data);
                           deferred.reject('Error fetching decks with tags ');
                       });
                   return deferred.promise;
            },
        };
    }]);
});
